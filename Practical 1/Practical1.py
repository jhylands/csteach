...
def threeDLeastSquares(X,Y):
    ...
    A=matrix([[2*ss(X[0]),Sxy(X[0],X[1]),sum(X[0])],[Sxy(X[0],X[1]),2*ss(X[1]),sum(X[1])],[sum(X[0]),0,2*len(X[0])],'double')
    c=matrix([[Sxy(X[0],Y)],[Sxy(X[1],Y)],[sum(Y)]],'double')
    #-------------------#
